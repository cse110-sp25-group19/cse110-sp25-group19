const { initGame, flipCard, GameState } = require('../game');
const { Card } = require('../utils');
...
test('flipCard on two non-matching cards resets combo and flips them back after 1s', () => {
  ...
  const { flippedCards, combo } = flipCard(1);

  // After flipping second card, flippedCards should be cleared immediately
